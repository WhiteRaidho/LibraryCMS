...
export default class AuthService {
  ...
  public static async login(
    ...
    return (
      await Vue.axios.post<AuthModel>("users/authenticate", {
        Username: username,
        Password: password
      })
    ).data;
  ...
  public static async recoverToken(refreshToken: string): Promise<TokenModel> {
    return (
      await Vue.axios.post<TokenModel>("users/authenticate/recover", {
        token: refreshToken
      })
    ).data;
  }
  ...
  public static async revokeToken(refreshToken: string): Promise<Statement> {
    return (
      await Vue.axios.post<Statement>("users/authenticate/revoke", {
        token: refreshToken
      })
    ).data;
  }
